...
func (c *NatsConnection) Open(ctx context.Context, correlationId string) error {
	...
	if err != nil {
		c.Logger.Error(ctx, correlationId, err, "Failed to connect to NATS server at "+uri)
		return err
	}

	c.Connection = connection

	c.Logger.Debug(ctx, correlationId, "Connected to NATS server at "+uri)
...
func (c *NatsConnection) Close(ctx context.Context, correlationId string) error {
	...
	c.Connection.Close()
	c.Logger.Debug(ctx, correlationId, "Disconnected to NATS server")
