...
func (c *NatsMessageQueue) Open(ctx context.Context, correlationId string) error {
	...
	err := c.NatsAbstractMessageQueue.Open(ctx, correlationId)
	if err != nil {
		return err
	}
...
func (c *NatsMessageQueue) Close(ctx context.Context, correlationId string) error {
	...
	err := c.NatsAbstractMessageQueue.Close(ctx, correlationId)
...
func (c *NatsMessageQueue) subscribe(ctx context.Context, correlationId string) error {
	c.Lock.Lock()
	defer c.Lock.Unlock()
	...
	if err != nil {
		c.Logger.Error(ctx, correlationId, err, "Failed to subscribe to subject "+subject)
		return err
	}
...
func (c *NatsMessageQueue) OnMessage(msg *nats.Msg) {
	...
	c.Logger.Debug(context.Background(), message.CorrelationId, "Received message %s via %s", msg, c.Name())
...
func (c *NatsMessageQueue) sendMessageToReceiver(receiver cqueues.IMessageReceiver, message *cqueues.MessageEnvelope) {
	...
	defer func() {
		if r := recover(); r != nil {
			err := fmt.Sprintf("%v", r)
			c.Logger.Error(context.Background(), correlationId, nil, "Failed to process the message - "+err)
		}
	}()

	err := receiver.ReceiveMessage(context.Background(), message, c)
	if err != nil {
		c.Logger.Error(context.Background(), correlationId, err, "Failed to process the message")
	}
}
