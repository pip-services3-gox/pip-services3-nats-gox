...
func (c *NatsBareMessageQueue) Receive(ctx context.Context, correlationId string, waitTimeout time.Duration) (*cqueues.MessageEnvelope, error) {
	...
	if msg != nil {
		...
		c.Logger.Debug(ctx, message.CorrelationId, "Received message %s via %s", msg, c.Name())
	...
	return func(msg *nats.Msg) {
		...
		func(message *cqueues.MessageEnvelope) {
			defer func() {
				if r := recover(); r != nil {
					err := fmt.Sprintf("%v", r)
					c.Logger.Error(ctx, message.CorrelationId, nil, "Failed to process the message - "+err)
				}
			}()

			err = receiver.ReceiveMessage(ctx, message, c)
			if err != nil {
				c.Logger.Error(ctx, message.CorrelationId, err, "Failed to process the message")
			}
		}(message)
	}
}
